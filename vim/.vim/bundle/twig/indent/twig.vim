" Contributed: Nelson Yeung

if exists("b:did_indent") | finish | endif

let templatedFileType = templatedExtensionRecognizer#guessTemplatedFiletype()
exec 'runtime! indent/'.templatedFileType.'.vim indent/'.templatedFileType.'_*.vim indent/'.templatedFileType.'/*.vim'

let b:baseIndentExpr=&indentexpr
...
function! s:CallBaseIndent()
    ...
    exe 'silent echo ' . b:baseIndentExpr
    exe 'redir END'
...
fun! s:BuildStructures()
    let structures = ['if','for','block', 'filter', 'image', 'javascripts', 'stylesheets']
